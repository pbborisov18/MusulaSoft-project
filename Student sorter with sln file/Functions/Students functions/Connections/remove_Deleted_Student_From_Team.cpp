...
void connectionsRemoveStudentFromTeam(string studentEmail){ //function responsible for deletion of the students email from Teams.csv
    ...
    while(getline(file,teamName,',')){ //If there is text between the start of the document and the first comma we enter the while() loop
        ...
        if(student1 != studentEmail && student2 != studentEmail && student3 != studentEmail && student4 !=studentEmail){ //If the student isn't part of the team we just rewrite the row
            ...
            if(student1 == studentEmail){   //If the first cell is the one searched for we enter the if statement
            ...
            if(student2 == studentEmail){   //If the second cell is the one searched for we enter the if statement
                fileCreate << record[0] << "," << record[1] << "," << " " << "," << record[3] << "," << record[4] << "," << record[5] << "\n";  //Writing the information we got earlier to the temporary file TeamsTemp.csv depending on which student we want to remove from Teams.csv
                record.clear(); //Clears the vector of all varibles. Resets it
            }

            if(student3 == studentEmail){   //If the third cell is the one searched for we enter the if statement
                fileCreate << record[0] << "," << record[1] << "," << record[2] << "," << " " << "," << record[4] << "," << record[5] << "\n";  //Writing the information we got earlier to the temporary file TeamsTemp.csv depending on which student we want to remove from Teams.csv
                record.clear(); //Clears the vector of all varibles. Resets it
            }

            if(student4 == studentEmail){   //If the fourth cell is the one searched for we enter the if statement
